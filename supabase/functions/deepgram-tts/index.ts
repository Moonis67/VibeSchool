...
const getRateLimitIdentity = async (req: Request) => {
...
const synthesizeDeepgramSpeech = async (text: string, voice: string) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Deepgram TTS failed with ${response.status}${detail ? `: ${detail.slice(0, 200)}` : ""}.`);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const rateLimitIdentity = await getRateLimitIdentity(req);
    if (!enforceRateLimit(rateLimitIdentity)) return jsonResponse(req, { error: safeErrorMessage(429) }, 429);
    ...
    for (const part of splitText(text)) {
      audioParts.push(await synthesizeDeepgramSpeech(part, voice));
    }
